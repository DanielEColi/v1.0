...
void GPIO_PinAFConfig(GPIO_TypeDef* GPIOx, uint16_t GPIO_PinSource, uint8_t GPIO_AF)
{
  ...
  assert_param(IS_GPIO_AF(GPIO_AF));
	...
	case GPIO_Pin_0:
		...
		break;
	case GPIO_Pin_1:
		GPIOx->AFR[0]&=0xffffff0f;
		GPIOx->AFR[0]|=(GPIO_AF<<4);
		break;
	case GPIO_Pin_2:
		GPIOx->AFR[0]&=0xfffff0ff;
		GPIOx->AFR[0]|=(GPIO_AF<<8);
		break;
	case GPIO_Pin_3:
		GPIOx->AFR[0]&=0xffff0fff;
		GPIOx->AFR[0]|=(GPIO_AF<<12);
		break;
	case GPIO_Pin_4:
		GPIOx->AFR[0]&=0xfff0ffff;
		GPIOx->AFR[0]|=(GPIO_AF<<16);
		break;
	case GPIO_Pin_5:
		GPIOx->AFR[0]&=0xff0fffff;
		GPIOx->AFR[0]|=(GPIO_AF<<20);
		break;
	case GPIO_Pin_6:
		GPIOx->AFR[0]&=0xf0ffffff;
		GPIOx->AFR[0]|=(GPIO_AF<<24);
		break;
	case GPIO_Pin_7:
		GPIOx->AFR[0]&=0x0fffffff;
		GPIOx->AFR[0]|=(GPIO_AF<<28);
		break;
	case GPIO_Pin_8:
		GPIOx->AFR[1]&=0xfffffff0;
		GPIOx->AFR[1]|=(GPIO_AF<<0);
		break;
	case GPIO_Pin_9:
		GPIOx->AFR[1]&=0xffffff0f;
		GPIOx->AFR[1]|=(GPIO_AF<<4);
		break;
	case GPIO_Pin_10:
		GPIOx->AFR[1]&=0xfffff0ff;
		GPIOx->AFR[1]|=(GPIO_AF<<8);
		break;
	case GPIO_Pin_11:
		GPIOx->AFR[1]&=0xffff0fff;
		GPIOx->AFR[1]|=(GPIO_AF<<12);
		break;
	case GPIO_Pin_12:
		GPIOx->AFR[1]&=0xfff0ffff;
		GPIOx->AFR[1]|=(GPIO_AF<<16);
		break;
	case GPIO_Pin_13:
		GPIOx->AFR[1]&=0xff0fffff;
		GPIOx->AFR[1]|=(GPIO_AF<<20);
		break;
	case GPIO_Pin_14:
		GPIOx->AFR[1]&=0x0fffffff;
		GPIOx->AFR[1]|=(GPIO_AF<<24);
		break;
	case GPIO_Pin_15:
		GPIOx->AFR[1]&=0x3fff;
		GPIOx->AFR[1]|=(GPIO_AF<<28);
		break;
	default:
		return;
	} 
...
//注意：：usart6 和spi在配置 管脚链接时，好像所调用的函数不一样，否则会出现错误，因此在
//此定义两个GPIO_PinAFConfig1，以便用各自能用的函数进行定义
void GPIO_PinAFConfig1(GPIO_TypeDef* GPIOx, uint16_t GPIO_PinSource, uint8_t GPIO_AF)
{
  uint32_t temp = 0x00;
  uint32_t temp_2 = 0x00;
