...
char CALIBRATION_ENABLED = 1;

const unsigned char ILED1_CURR_MAX_mA = 90;     // LED1 max current reqd. for application
const unsigned char ILED1_CURR_MIN_mA = 50;      // LED1 min current reqd. for application
const unsigned char ILED2_CURR_MAX_mA = 90;     // LED2 max current reqd. for application
const unsigned char ILED2_CURR_MIN_mA = 50;      // LED2 min current reqd. for application
...
const unsigned char TARGET_THR_PERCENT = 40;    // Target Threshold percent
/* End of Variables to modify depending on the application */
calibRoutineParams calibParams;

//初始化电流增益等相关指标，比如说最小电流，最大电流，ad采样的阀值
void InitCurrentGainIndicators(void)
{
...
void calibrationLED1(unsigned long LED1data)
{
  ...
  case (sInit) :
    ...
    LED1InterimCode = (ILED1_CURR_MAX_Code<<8);
    ...
    WriteCommand((unsigned char)TIAGAIN, AFE44xx_Current_Register_Settings[32]);                                //0x20
    WriteCommand((unsigned char)LEDCNTRL, AFE44xx_Current_Register_Settings[34]);                               //0x22
  ...
  case (sWaitTime1) :
    ...
    // is the peak value from the BLOCK size sample data. 
  case (sBlock1) :
    LED1_blockSizeCount++;
    LED1_Meas_DC += LED1data;
    //    // The DC value is the peak value from the BLOCK size sample data.
  ...
  case (sBlock2) :
    ...
    if (LED1_blockSizeCount == BLOCK_SIZE)
    {
      ...
      {
				if(LED1_Meas_DC > HIGH_THR_HYS_HIGH && LED1_DISABLE_HIGH_CHECK == TRUE)
				{
					ILED1Code = ((AFE44xx_Current_Register_Settings[34]&0xff00) >> 8) - 20;
					if (ILED1Code > ILED1_CURR_MAX_Code)
						ILED1Code = ILED1_CURR_MIN_Code;
					LED1InterimCode = (ILED1Code<<8);
					AFE44xx_Current_Register_Settings[34] = (AFE44xx_Current_Register_Settings[34] & ~(LED1_CURRENT)) | LED1InterimCode;
					Disable_AFE44xx_SPI_Read();   
					WriteCommand((unsigned char)LEDCNTRL, AFE44xx_Current_Register_Settings[34]);    
				}
...
void calibrationLED2(unsigned long LED2data)
{
  ...
  case (sInit) :
    ...
    AFE44xx_Current_Register_Settings[34] = (AFE44xx_Current_Register_Settings[34] & ~(LED2_CURRENT)) | (ILED2_CURR_MAX_Code);
    AFE44xx_Current_Register_Settings[33] &= ~STAGE2EN_LED2;
    Disable_AFE44xx_SPI_Read();
    WriteCommand((unsigned char)TIA_AMB_GAIN, AFE44xx_Current_Register_Settings[33]);                           //0x21
    WriteCommand((unsigned char)LEDCNTRL, AFE44xx_Current_Register_Settings[34]);                               //0x22
    Enable_AFE44xx_SPI_Read();
      ...
      if (LED2_Meas_DC > TARGET_THR)
      {
        ...
        if (LED2RFValue > s10K)
        {
          ...
          case (s50K) :
            ...
            Disable_AFE44xx_SPI_Read();
            WriteCommand((unsigned char)TIA_AMB_GAIN, AFE44xx_Current_Register_Settings[33]);                                //0x20
            Enable_AFE44xx_SPI_Read();
            break;
          case (s25K) :
            // Set TIA gain to 25K.
  ...
  case (sBlock2) :
    ...
    if (LED2_blockSizeCount == BLOCK_SIZE)
    {
      ...
      {
				if(LED2_Meas_DC > HIGH_THR_HYS_HIGH && LED2_DISABLE_HIGH_CHECK == TRUE)
				{
					ILED2Code = ((AFE44xx_Current_Register_Settings[34]&0x00ff) - 20);
					if (ILED2Code > ILED2_CURR_MAX_Code)
						ILED2Code = ILED2_CURR_MIN_Code;
					AFE44xx_Current_Register_Settings[34] = (AFE44xx_Current_Register_Settings[34] & ~(LED2_CURRENT)) | ILED2Code;
					Disable_AFE44xx_SPI_Read();   
					WriteCommand((unsigned char)LEDCNTRL, AFE44xx_Current_Register_Settings[34]);    
				}				
